...
seatsRemaining and totalTravelDistance affect the price of a plane ticket (totalFare) and used Multiple Linear Regression to do this. The original data is a 31GB file of flight tickets found on Expedia between 04/16/2022 and 10/05/2022.
...
The highest R-squared value of the 16 models is 43.82% and the lowest is 8.73%. These low values of R-squared are most likely due to the temporal effect limitation discussed above. Nonetheless, both variables, seatsRemaining and totalTravelDistance, are significant in each model. This allows us to conclude that our predictors affect ticket price for all 16 models. 

The coefficients for each of the 16 models vary. About half of the models have a negative relationship between seatsRemaining and totalFare, whereas the other half is positive. This could be due to an airline wanting to maximize revenue. For example, if a plane is not at full capcaity, they could charge more in order to make up for the seats that are vacant.  Each model also has a high intercept.


## Condor Details  

We created a shell script to split the large 31GB dataset, filtered by basic economy tickets, and removed unnecessary columns to reduce memory, leaving us with 16 files of size 20 to 30 MB. Afterwards, we tested our R script by running a regression model on only one csv (ATL) to ensure our scripts were working before running them on all 16 files. We initially used 1 GB of memory to run our first job but got an error message stating that more memory was required to run the job.We increased our memory to 5 GB but that also didn’t work. We realized that lm() requires a lotof memory to run in CHTC so we ended up increasing our memory to 8 GB to get just one job to run. Once our first job was successful, we ran three more jobs and then finally, ran all 16 jobs which took about 6 minutes to run in total and required 20 GB of memory along with 2 GB of disk space.  
